...
Usage: ./installation_no_tracing.sh [-s] [-b] [-r] [--] [args...]
  
Examples:
  ./installation_no_tracing.sh -s
  ./installation_no_tracing.sh -sb            # same as -s -b
  ./installation_no_tracing.sh -bs -- file1   # flags + positional
  ./installation_no_tracing.sh -r             # rebuild only failed packages from unable_to_build.txt
EOF
}
...
PY="${PYTHON:-python3}"
USE_CUDA_SUFFIX="${USE_CUDA_SUFFIX:-1}"
...
run_build_autoware_clean() {
	...
	env -i \
		...
		bash -lc '
			set -e
			if [ -n "$1" ]; then
				"${SCRIPT_DIR}/build_autoware_no_tracing.sh" "${SCRIPT_DIR}" "$0" "$1"
			else
				"${SCRIPT_DIR}/build_autoware_no_tracing.sh" "${SCRIPT_DIR}" "$0"
			fi
